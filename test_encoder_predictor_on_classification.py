'''This module here is intended to test/debug the capability of the encoder network purely on the classification problem.
'''
...
def downstream(args):
    ...
    summary(PREDICTOR_NETWORK, (int(NUM_PATCHES), int(ENCODER_NETWORK_EMBEDDING_DIM)))
    ...
    logger.info(f"Total train dataset: {len(TRAIN_DATASET_MODULE)}")

    OPTIMIZER = AdamW(list(ENCODER_NETWORK.parameters()) + list(PREDICTOR_NETWORK.parameters()), lr=LEARNING_RATE)
    ...
    for epoch_idx in range(START_EPOCH, END_EPOCH):
        ...
        for train_idx, data in tqdm(enumerate(TRAIN_DATASET_LOADER)):
            ...
            with torch.cuda.amp.autocast(dtype=torch.bfloat16, enabled=USE_BFLOAT16):
                ...
                prediction = ENCODER_NETWORK(batch_x)
                prediction = PREDICTOR_NETWORK(prediction)
